...
# import torch.hub.torch.hub.load_state_dict_from_url as torch.hub.load_state_dict_from_url
...
def iresnet18(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(torch.hub.load_state_dict_from_url(model_urls['iresnet18'],
...
def iresnet34(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(torch.hub.load_state_dict_from_url(model_urls['iresnet34'],
                                                           root=default_cache_path)))
    return model
...
def iresnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.hub.load_state_dict_from_url(model_urls['iresnet50']))
    return model
...
def iresnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.hub.load_state_dict_from_url(model_urls['iresnet101']))
    return model
...
def iresnet152(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(torch.hub.load_state_dict_from_url(model_urls['iresnet152'],
                                                           root=default_cache_path)))
    return model
...
def iresnet200(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(torch.hub.load_state_dict_from_url(model_urls['iresnet200'],
                                                           root=default_cache_path)))
    return model
...
def iresnet302(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(torch.hub.load_state_dict_from_url(model_urls['iresnet302'],
                                                           root=default_cache_path)))
    return model
...
def iresnet404(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(torch.hub.load_state_dict_from_url(model_urls['iresnet404'],
                                                           root=default_cache_path)))
    return model
...
def iresnet1001(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(torch.hub.load_state_dict_from_url(model_urls['iresnet1001'],
                                                           root=default_cache_path)))
    return model